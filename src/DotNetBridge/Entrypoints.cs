...
namespace Microsoft.ML.DotNetBridge
{
    internal static class DotNetBridgeEntrypoints
    {
        ...
        public sealed class ApplyExTransformModelInputNew : TransformInputBase
        {
            [Argument(ArgumentType.Required, HelpText = "Predictor model", SortOrder = 1)]
            public PredictorModel PredictorModel;
        }
        ...
        [TlcModule.EntryPoint(Name = "Models.DatasetTransformerEx", Desc = "Applies a TransformModel to a dataset.", UserName = "Apply Transform Model Output")]
        public static ApplyExTransformModelOutput Apply(IHostEnvironment env, ApplyExTransformModelInputNew input)
        {
            Contracts.CheckValue(env, nameof(env));
            var host = env.Register("TransformModel");
            host.CheckValue(input, nameof(input));
            EntryPointUtils.CheckInputArgs(host, input);
            ...
            // If the given model includes one or more predictors, then Predictor Model should work.
            // Else (no predictors in given model), error should be caught, and TransformModelOutput() should be called.
            try
            {
                IDataView outputData;
                input.PredictorModel.PrepareData(host, input.Data, out data, out predictor);
                using (var ch = host.Start("Creating scoring pipeline"))
                {
                    ch.Trace("Creating pipeline");
                    var bindable = ScoreUtils.GetSchemaBindableMapper(host, predictor);
                    ...
                    outputData = scorer.CreateComponent(host, data.Data, mapper, input.PredictorModel.GetTrainingSchema(host));
                }
                return new ApplyExTransformModelOutput() { OutputData = outputData };
            }
            catch (Exception)
            {
                return new ApplyExTransformModelOutput() { OutputData = input.PredictorModel.TransformModel.Apply(env, input.Data) };
            }
        }
    }
}
